...
//        $this->checkAnonymous();
        ...
        $Wish = new wishModel();
        $list = $Wish->where(['type' => 1,'status' => 0])->order('create_time desc')->limit(7)->select();  // 活动列表
...
//        $this->checkAnonymous();
        ...
        if ($type == 1){
            // 活动  列表
            $list = $Wish->where(['type' => 1,'status' => 0])->order('create_time desc')->limit($len,5)->select();  // 活动列表
