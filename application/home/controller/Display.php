...
use app\home\model\DisplayVote;
...
class Display extends Base{
    ...
    public function indexmore(){
        ...
        $list = DisplayModel::where($map)->order('id desc')->limit($len,5)->select();
    ...
    public function vote(){
        ...
        if(empty($Vote->where($data)->find())){
            $res = $Vote->save($data);
            if ($res){
                DisplayModel::where(['id' => $id,'status' => 0])->setInc('votes');
