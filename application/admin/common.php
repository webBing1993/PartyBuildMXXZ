...
function is_administrator($uid = null){
    ...
    return $uid && (intval($uid) === \think\Config::get('user_administrator'));
}
...
function time_date($time) {
    ...
    if($tmp < 60) {
        ...
        $result = floor($tmp/60).'分钟前';
    } else if($tmp < 86400) {
        $result = floor($tmp/3600).'小时前';
    } else if($tmp < 259200) {//3天内
        $result = floor($tmp/86400).'天前';
    } else {
...
function get_nickname($uid = 0){
    ...
    /* 获取缓存数据 */
    if(empty($list)){
        $list = \think\Cache::get('sys_user_nickname_list');
    }
    ...
    if(isset($list[$key])){ //已缓存，直接使用
        ...
        $info = \app\admin\model\Member::get($uid);
        if($info !== false && $info['nickname'] ){
            ...
            \think\Cache::set('sys_user_nickname_list', $list);
        } else {
            $name = '';
        }
    }
 ...
 * @return array
 */
function array_to_id($array, $field='id') {
 ...
 * @return array
 */
function array_id_array($array, $field='id') {
    foreach($array as $key=>$value){
...
function get_config_type($type=0){
    $list = think\Config::get('CONFIG_TYPE_LIST');
    return $list[$type];
}
...
function get_config_group($group=0){
    $list = think\Config::get('CONFIG_GROUP_LIST');
...
function wechat_status_to_string(&$data) {
    if($data === false || $data === null ){
        return $data;
    }
    $data = (array)$data;
...
function get_action($id = null, $field = null){
    ...
    $list = \think\Cache::get('action_list');
    if(empty($list[$id])){
        $map = array('status'=>array('gt', -1), 'id'=>$id);
        \app\admin\model\Action::get(function($query) use($map){
            $query->where($map);
        });
    }
 ...
 * @return string
 */
function array_to_string($array, $field='id', $separator=',') {
    foreach($array as $key=>$value){
        $data[$key] =  $value[$field];
    }
