...
use com\ueditor\UEditor;
use think\Config;
use think\Controller;
use Think\Db;
use think\Input;
use org\Database as DatabaseLib;
...
class Database extends Admin {
    ...
    public function export($tables = null, $id = null, $start = null) {
        if (IS_POST) {
            if (!empty($tables) && is_array($tables) && input("type") != 2) {
                //初始化
                ...
                //创建备份文件
                $DatabaseLib = new DatabaseLib($file, $config);
                if (false !== $DatabaseLib->create()) {
                    $tab = array('id' => 0, 'start' => 0, 'type' => 2);
                    return $this->success('初始化成功！', '', array('tables' => $tables, 'tab' => $tab));
                } else {
                    return $this->error('初始化失败，备份文件创建失败！');
                }
            } elseif (is_numeric($id) && is_numeric($start) && input("type") == 2) {
                //备份数据
                $tables = session('backup_tables');
                //备份指定表
                $DatabaseLib = new DatabaseLib(session('backup_file'), session('backup_config'));
                $start = $DatabaseLib->backup($tables[$id], $start);
                if (false === $start) {
                    //出错
                    ...
                    //下一表
                    if (isset($tables[++$id])) {
                        $tab = array('id' => $id, 'start' => 0, 'type' => 2);
                    ...
                    $tab = array('id' => $id, 'start' => $start[0], 'type' => 2);
    ...
    public function import($time = 0, $part = null, $start = null) {
        if (IS_POST) {
            ...
            if (is_numeric($time) && is_null($part) && is_null($start)) {
                ...
                $db = new DatabaseLib($list[$part], array(
                    'path' => realpath($path) . DIRECTORY_SEPARATOR,
